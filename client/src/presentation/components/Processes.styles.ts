...
export const ProcessesContainer = styled.div<{ isVisible: boolean }>`
  min-height: 100vh;
  ...
  transform: ${props => props.isVisible ? 'translateX(0) scale(1)' : 'translateX(20px) scale(0.98)'};
  transition: all 0.6s cubic-bezier(0.23, 1, 0.32, 1);
  pointer-events: ${props => props.isVisible ? 'auto' : 'none'};
  overflow: hidden;
`;

export const ProcessesContent = styled.div`
  position: relative;
  width: 100%;
  height: 100vh;
  display: flex;
